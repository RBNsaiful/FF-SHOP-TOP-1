...
const WifiOffIcon: FC<{className?: string}> = ({className}) => (
...
const VideoAdPlayer: FC<VideoAdPlayerProps> = ({ videoUrl, onComplete, onClose, duration = 15 }) => {
    ...
    const [isOffline, setIsOffline] = useState(false); // Track connectivity
    ...
    useEffect(() => {
        const checkConnection = () => {
            if (!navigator.onLine) {
                setIsOffline(true);
                // Adjust end time so time doesn't "pass" while offline
    ...
    // Timer Logic - Only runs when isContentReady is TRUE and ONLINE
    useEffect(() => {
        if (!isContentReady || canSkip || hasError || isOffline) return;
    ...
    }, [isContentReady, canSkip, duration, onComplete, hasError, isOffline]);
        ...
        <div className="fixed inset-0 z-[100] bg-black flex flex-col justify-center items-center">
            ...
            <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-20 bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
                <div className={`px-4 py-1.5 rounded-full text-xs font-bold border backdrop-blur-md shadow-lg transition-colors ${
                    hasError || isOffline ? 'bg-red-500/90 border-red-400 text-white' :
            ...
            <div className="relative w-full h-full flex items-center justify-center bg-black">
                ...
                {isOffline && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-30 bg-black/90 p-6 text-center">
                        <div className="w-16 h-16 rounded-full bg-red-900/30 flex items-center justify-center mb-4">
                            <WifiOffIcon className="w-8 h-8 text-red-500" />
                        </div>
                        <h3 className="text-lg font-bold mb-2">No Internet Connection</h3>
                        <p className="text-sm text-gray-400 mb-6 max-w-xs mx-auto">
                            Please restore your internet connection to continue earning rewards.
                        </p>
                    </div>
                )}
